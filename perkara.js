...
// fitBounds() bawaan Leaflet men-zoom out sampai SELURUH bounds muat di dalam
// viewport ("contain") — kalau bentuk kontainer (landscape/lebar) tidak sama
// rasio dengan bentuk wilayah hukum (cenderung portrait), sisa ruang di kiri/
// kanan jadi abu-abu kosong (persis seperti letterbox). prkFitCover() sebaliknya
// men-zoom secukupnya agar peta memenuhi seluruh kontainer ("cover"), dengan
// konsekuensi sedikit sisi terluar wilayah bisa terpotong — tidak ada lagi area abu-abu.
function prkFitCover() {
  if (!prkMap) return;
  const targetZoom = prkMap.getBoundsZoom(PRK_BOUNDS, true);
  prkMap.setView(PRK_CENTER, Math.max(targetZoom, prkMap.getMinZoom()));
}
...
function initPrkMap() {
  ...
  try {
    ...
    // peta akan invalidateSize() + prkFitCover() setiap kali ukuran sebenarnya
    // dari #prkMap berubah (mis. saat layout/flex masih menata, atau jendela
    // di-resize) — supaya area abu-abu tidak muncul lagi kapan pun ukurannya berubah.
    if ('ResizeObserver' in window) {
      const ro = new ResizeObserver(() => {
        if (prkMap) { prkMap.invalidateSize(true); prkFitCover(); }
      });
